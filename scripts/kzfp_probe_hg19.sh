...
echo "STEP 1: Extract canonical basic transcripts from GENCODE v19..."
grep 'transcript' "$GTF" | grep 'tag "basic"' \
    | sed -E 's/.*transcript_id "([^"]+)".*/\1/' \
    | sort -u > "$OUTDIR/kzfp_basic_tids.txt"

# Keep only exons that belong to these canonical transcripts
awk '
    BEGIN {
        while ((getline < "'"$OUTDIR/kzfp_basic_tids.txt"'") > 0) tids[$1]=1
    }
    $3=="exon" {
        match($0, /transcript_id "([^"]+)"/, t)
        if (tids[t[1]]) print $0
    }
' "$GTF" > "$OUTDIR/kzfp_basic_exons_hg19.gtf"

echo "STEP 1 complete"


echo "STEP 2: Filter canonical exons to only KZFP genes..."
awk '
BEGIN {
...
}' "$OUTDIR/kzfp_basic_exons_hg19.gtf" > "$OUTDIR/kzfp_all_exons_hg19.gtf"
echo "STEP 2 complete"


echo "STEP 3: Convert exon GTF to BED..."
awk 'BEGIN {OFS="\t"} {
