...
}' "$GTF" | sort -u > "$OUTDIR/kzfp_canonical_transcripts_hg19.txt"
echo "STEP 1 ✓"

echo "STEP 2: Extract canonical exons..."
awk '
BEGIN {
    while ((getline < "'"$OUTDIR/kzfp_canonical_transcripts_hg19.txt"'") > 0) tx[$1]=1
}
$3=="exon" {
    match($0, /transcript_id "([^"]+)"/, t)
    if (tx[t[1]]) print $0
}' "$GTF" > "$OUTDIR/kzfp_canonical_exons_hg19.gtf"
echo "STEP 2 ✓"
...
}' "$OUTDIR/kzfp_canonical_exons_hg19.gtf" | sort -k1,1 -k2,2n > "$OUTDIR/kzfp_exons_raw_hg19.bed"
echo "STEP 3 ✓"
